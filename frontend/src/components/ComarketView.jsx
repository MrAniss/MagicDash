...
function BrandSelector({ options, selected, onSelect }) {
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      >
        <span className="text-[10px] text-navy-muted font-bold uppercase tracking-wider">Marque:</span>
        <span className="text-xs font-semibold text-navy truncate">{selectedLabel}</span>
        <svg className={`w-3.5 h-3.5 text-navy-muted ml-auto transition-transform ${isOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="absolute right-0 mt-1 w-full bg-white border border-border-strong shadow-xl rounded-card overflow-hidden z-[60] animate-in fade-in zoom-in-95 duration-100">
          <div className="max-h-[300px] overflow-y-auto py-1">
            ...
            {options.map(brand => (
              <button
                key={brand}
                onClick={() => { onSelect(brand); setIsOpen(false); }}
...
const KPI_CARDS = [
  ...
  { key: 'spend',       label: 'COÛT',          format: fEurCompact,  deltaKey: 'spend_pct',        deltaType: 'pct', accent: '#F5A623' },
  ...
  { key: 'revenue',     label: 'REVENUE',       format: fEurCompact,  deltaKey: 'revenue_pct',      deltaType: 'pct', accent: '#00B87A' },
    ...
    <div className="space-y-4 text-navy">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-medium px-2.5 py-1 rounded-[6px] bg-warning-bg text-warning border border-warning/10">FR uniquement</span>
        </div>
          ...
          return (
            <div key={kpi.key} className="bg-white rounded-card border border-border shadow-card overflow-hidden">
              ...
              <div className="px-4 py-3">
                ...
                <p className={`text-[11px] font-medium ${isPos ? 'text-success' : isNeg ? 'text-danger' : 'text-navy-muted'} mb-0.5`}>
                ...
                {(kpi.key === 'spend' || kpi.key === 'revenue') && pctOfFR && (
                  <p className="text-[10px] text-navy-muted mt-0.5 font-medium">{pctOfFR[kpi.key]}% du total FR</p>
                )}
              </div>
            </div>
          );
      ...
      <CostKpiChart filters={filters} onlyComarket={true} partnerBrand={selectedBrand} title={`Trend CoMarket ${selectedBrand !== 'ALL' ? `- ${selectedBrand}` : ''}`} />
          ...
          <table className="w-full text-[13px] table-fixed">
            ...
            <tbody>
              {sortedBrands.map((brand, i) => {
                ...
                return (
                  <Fragment key={brand.label}>
                    <tr className={`border-b border-border cursor-pointer transition-all duration-150 ${isExpanded ? 'bg-navy text-white' : i % 2 === 1 ? 'bg-[#FAFBFD]' : 'hover:bg-bg-page'}`}
                      ...
                      {TABLE_COLS.map(col => {
                        ...
                        if (col.key === 'label') {
                          return (
                            <td key={col.key} className="px-3 py-3 whitespace-nowrap text-left font-bold truncate">
                              <div className="flex items-center gap-2">
                                <svg className={`w-2.5 h-2.5 transition-transform duration-200 ${isExpanded ? 'rotate-90 text-white/70' : 'text-navy-muted'}`} fill="currentColor" viewBox="0 0 20 20">
                                  <path d="M6 5l8 5-8 5V5z" />
                                </svg>
                                {val}
                                <span className={`ml-2 text-[9px] px-1.5 py-0.5 rounded-full font-bold tracking-wider ${isExpanded ? 'bg-white/20 text-white' : 'bg-navy/10 text-navy'}`}>
                        ...
                        return (
                          <td key={col.key} className="px-3 py-3 whitespace-nowrap text-right">
                            ...
                            {col.delta && dVal != null && (
                              <div className={`text-[10px] font-semibold ${isExpanded ? 'text-white/70' : isPos ? 'text-success' : isNeg ? 'text-danger' : 'text-navy-muted'}`}>
                    ...
                    {isExpanded && brand.campaigns.map((camp, ci) => (
                      <tr key={`${brand.label}-camp-${ci}`} className="border-b border-border bg-mint-bg/30 text-navy">
                        {TABLE_COLS.map(col => {
                          ...
                          return (
                            <td key={col.key} className={`px-3 py-2.5 whitespace-nowrap ${col.align === 'right' ? 'text-right' : 'text-left pl-8'}`}>
                              <div className={col.key === 'label' ? 'text-[11px] truncate italic max-w-full font-medium' : 'font-medium'}>{formatted}</div>
                              {col.delta && dVal != null && (
                                <div className={`text-[10px] font-semibold ${isPos ? 'text-success' : isNeg ? 'text-danger' : 'text-navy-muted'}`}>
                                  {isPos ? '▲' : isNeg ? '▼' : ''} {fDeltaVal(dVal)}
                                </div>
                              )}
                            </td>
              ...
              {sortedBrands.length === 0 && (
                <tr><td colSpan={TABLE_COLS.length} className="px-4 py-8 text-center text-navy-muted">Aucune marque partenaire trouvee</td></tr>
              )}
            </tbody>
          </table>
        </div>
