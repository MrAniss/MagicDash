...
import { fEur, fNum, fPct, fROAS, fAov } from '../utils/formatters';
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="bg-white rounded-card p-4 border border-border shadow-card flex items-center gap-4 flex-wrap">
          <span className="text-xs text-navy flex items-center gap-1.5">
            ...
            <button
              ...
              className="text-navy-muted hover:text-navy ml-1 text-sm"
            >
              &#10005;
            </button>
          </span>
      ...
      <div className="bg-white rounded-card border border-border shadow-card overflow-hidden">
        <div className="px-6 py-5 pb-3 flex items-center justify-between">
          ...
          <ExportButtons
            ...
            onSheets={async () => {
              await copyTsv(TYPE_TABLE_COLS, typeSummary);
              setCopiedType(true);
              setTimeout(() => setCopiedType(false), 2000);
            }}
            copied={copiedType}
          />
        </div>
            ...
            <thead>
              <tr className="bg-bg-page border-b-2 border-border">
                {TYPE_TABLE_COLS.map((col) => (
                  <th
                    key={col.key}
                    className={`px-3 py-3 text-[11px] font-semibold text-navy-muted uppercase tracking-[0.06em] whitespace-nowrap ${col.align === 'right' ? 'text-right' : 'text-left'}`}
                  >
                    {col.label}
                  </th>
                ))}
                  ...
                  >
                    {TYPE_TABLE_COLS.map((col) => {
                      ...
                      if (col.key === 'roas') {
                        cls +=
                          ' font-bold ' +
                          (val >= 4 ? 'text-success' : val >= 2.5 ? 'text-warning' : 'text-danger');
                        ...
                        <td key={col.key} className={cls}>
                          <div className={col.align === 'right' ? 'flex flex-col items-end' : ''}>
                            ...
                            {col.hasDelta && deltaVal !== undefined && (
                              <span
                                className={`text-[10px] font-medium ${isSelected ? deltaColor(deltaVal) : `group-hover:text-white ${deltaColor(deltaVal)}`} leading-tight`}
                              >
                                {deltaVal > 0 ? '▲' : deltaVal < 0 ? '▼' : ''}{' '}
      ...
      <div className="grid grid-cols-3 gap-4">
        {/* Spent Donut */}
        <div className="bg-white rounded-card p-6 border border-border shadow-card">
          <h3 className="text-[11px] font-semibold text-navy-muted uppercase tracking-wider mb-4 text-center">
            Repartition spend
          </h3>
                ...
                >
                  {pieSpendData.map((entry, i) => (
                    <Cell
                      ...
                      stroke={activeType === entry.name ? '#1A2E4A' : 'transparent'}
                      strokeWidth={activeType === entry.name ? 2 : 0}
                    />
                  ))}
                </Pie>
                <Tooltip
          ...
          <div className="h-[180px] mt-4">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                />
                <Tooltip
                  cursor={{ fill: 'rgba(26,46,74,0.02)' }}
                  contentStyle={{
                    background: '#FFFFFF',
                    border: '1px solid rgba(26,46,74,0.15)',
                    borderRadius: 12,
                ...
                >
                  {typeSummary.map((entry, index) => (
                    <Cell
                      ...
                      fillOpacity={activeType && activeType !== entry.type ? 0.3 : 1}
                    />
                  ))}
                </Bar>
              </BarChart>
          ...
          <div className="flex items-center gap-4">
            ...
            {selectedCampaigns.length > 0 && (
              <span className="text-[11px] text-navy-muted">
                {selectedCampaigns.length}/5 selectionnees pour comparaison ROAS
              </span>
            )}
          </div>
          <ExportButtons
            ...
            onSheets={async () => {
              await copyTsv(TABLE_COLS, sorted);
              setCopiedCamps(true);
              setTimeout(() => setCopiedCamps(false), 2000);
            }}
            copied={copiedCamps}
          />
        </div>
            ...
            <tbody>
              {sorted.map((row, i) => {
                ...
                return (
                  <tr
                    key={i}
                    className={`border-b border-border hover:bg-navy/5 transition-colors group ${isSelected ? 'bg-mint-bg' : i % 2 === 1 ? 'bg-[#FAFBFD]' : ''}`}
                  >
                    <td className="px-2 py-2.5">
                      <input
                        type="checkbox"
                        checked={isSelected}
                        onChange={() => toggleCampaign(row.campaign_name)}
                        ...
                        <td key={col.key} className={cls} title={col.wide ? val : undefined}>
                          ...
                          >
                            {col.key === 'campaign_name' && (
                              <TypeBadge type={row.type} color={typeColors[row.type]} />
                            )}
                            <span className="leading-tight">{formatted}</span>
                            {col.hasDelta && deltaVal !== undefined && (
                              <span
                                className={`text-[10px] font-medium ${deltaColor(deltaVal)} leading-tight`}
                              >
                                {deltaVal > 0 ? '▲' : deltaVal < 0 ? '▼' : ''}{' '}
                                {Math.abs(deltaVal).toFixed(1)}%
                              </span>
              ...
              {sorted.length === 0 && (
                <tr>
                  <td
                    ...
                    className="px-4 py-8 text-center text-navy-muted"
                  >
